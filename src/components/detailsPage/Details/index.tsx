	...
	return (
		<div className="bg-[rgba(50,50,50,0.6)] mx-auto mt-2 p-2 px-4 max-w-[600px]">
			...
			<h1 className="text-emerald-500 font-semibold text-2xl flex justify-center items-center py-2 ">
				{data.mission_name}
				{data.launch_success && <i className="bx bxs-check-circle pl-2"></i>}
			</h1>
			<div>
				<p className="text-center text-emerald-600">{data.details}</p>
				<ul className="text-[#ccc] mt-2">
					<li>Rocket: {data.rocket.rocket_name}</li>
					<li>Rocket Type: {data.rocket.rocket_type}</li>
					<li>Launch Year: {data.launch_year}</li>
					<li>Launch Site: {data.launch_site.site_name}</li>
					<li>Launch Status: {data.launch_success ? 'Success' : 'Failure'}</li>
					{!data.launch_success && (
						<li>Failure Reason: {data.launch_failure_details.reason}</li>
					)}
				</ul>
